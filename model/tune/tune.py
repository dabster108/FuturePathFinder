...
def scale_features(X_train: pd.DataFrame, X_test: pd.DataFrame, numeric_features: list):
    scaler = StandardScaler()
    X_train_scaled = X_train.copy()
    X_test_scaled = X_test.copy()
    ...
    return X_train_scaled, X_test_scaled, scaler
# To standardize (scale) the numeric features in the training and 
# testing datasets so they have mean 0 and standard deviation 1
#Transforms the test data’s numeric features using the same mean and
#  std dev learned from training data

def save_plot(plt, filename: str, folder_path: str):
    os.makedirs(folder_path, exist_ok=True)
    full_path = os.path.join(folder_path, filename)
    plt.savefig(full_path, bbox_inches='tight')
    print(f"Saved plot: {full_path}")
...
# stratify=y ensures that the split preserves the original distribution of the 
# target classes in both sets (important for balanced representation).
# random_state=42 fixes the randomness for reproducibility.
    ...
    param_grid = {
        ...
        'min_samples_split': [2, 5, 10],
        'min_samples_leaf': [1, 2, 4],
        'max_features': ['sqrt', 'log2', None]  # Added max_features to tune
    }
    ...
    # Convert None to str 'None' for plotting clarity
    results['param_max_depth'] = results['param_max_depth'].apply(lambda x: 'None' if x is None else str(x))
    results['param_max_features'] = results['param_max_features'].astype(str)

    # Heatmap: max_depth vs n_estimators (averaged over other params)
    pivot = results.pivot_table(index='param_max_depth', columns='param_n_estimators', values='mean_test_score')
    plt.figure(figsize=(8, 6))
    sns.heatmap(pivot, annot=True, cmap='viridis')
    plt.title('Accuracy Heatmap: max_depth vs n_estimators')
    ...
    # Plot mean accuracy vs max_depth for different n_estimators
    plt.figure(figsize=(10, 6))
    for n_est in sorted(results['param_n_estimators'].unique()):
        subset = results[results['param_n_estimators'] == n_est]
        plt.plot(subset['param_max_depth'], subset['mean_test_score'], marker='o', label=f"n_estimators={n_est}")
    ...
    # Plot mean accuracy vs min_samples_split for different min_samples_leaf
    plt.figure(figsize=(10, 6))
    for leaf in sorted(results['param_min_samples_leaf'].unique()):
        subset = results[results['param_min_samples_leaf'] == leaf]
        plt.plot(subset['param_min_samples_split'], subset['mean_test_score'], marker='o', label=f"min_samples_leaf={leaf}")
    ...
    # Plot mean accuracy vs max_features for different max_depths
    plt.figure(figsize=(10, 6))
    for depth in sorted(results['param_max_depth'].unique()):
        subset = results[results['param_max_depth'] == depth]
        plt.plot(subset['param_max_features'], subset['mean_test_score'], marker='o', label=f"max_depth={depth}")
    plt.xlabel('max_features')
    plt.ylabel('Mean CV Accuracy')
    plt.title('Grid Search: max_features vs Accuracy for different max_depth')
    plt.legend()
    plt.grid(True)
    save_plot(plt, 'max_features_vs_accuracy_by_max_depth.png', tune_vis_folder)
    plt.close()

    # Bar Chart: Top 10 parameter sets by accuracy
    top_n = results.sort_values(by='mean_test_score', ascending=False).head(10)
    plt.figure(figsize=(10, 6))
    plt.barh(range(len(top_n)), top_n['mean_test_score'], color='skyblue')
    plt.yticks(range(len(top_n)), top_n['params'])
    ...
    # Scatter plot max_depth vs n_estimators colored by accuracy
    plt.figure(figsize=(10, 6))
    results['param_max_depth'] = results['param_max_depth'].apply(lambda x: -1 if x == 'None' else int(x))
    scatter = plt.scatter(
        results['param_max_depth'],
        results['param_n_estimators'],
        c=results['mean_test_score'],
